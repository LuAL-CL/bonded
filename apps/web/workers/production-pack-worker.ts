...
new Worker<ProductionPackJobPayload>(
  ...
  async (job) => {
    await markJobRunning(job.id!, "PRODUCTION_PACK", job.attemptsStarted);
    ...
    try {
      ...
      await prisma.productionPack.create({
        data: { orderId: job.data.orderId, zipUrl: zipPath, version: "1.0.0" }
      });
  ...
  { connection: redisConnection, concurrency: Number(process.env.PRODUCTION_PACK_WORKER_CONCURRENCY ?? 1) }
);
